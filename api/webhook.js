      ...
      for (const entry of body.entry) {
        ...
        try {
          ...
          if (webhookEvent.message && !webhookEvent.message.is_echo) {
            ...
            // Default AI Chat with Smart Language Detection & Enforcement
...
async function handleCommandAction(senderPsid, input, apiKeys, pageToken) {
  ...
  if (['/english', '/eng'].includes(lowerText)) {
    try {
      await kv.set(`user_lang_${senderPsid}`, 'ENGLISH');
    } catch (e) {}
    await sendTextMessage(senderPsid, "🔤 **Language set to English!** From now on, I will respond strictly in English regardless of your input language.", pageToken);
    return true;
  }

  if (['/tagalog', '/filipino', '/tag'].includes(lowerText)) {
    try {
      await kv.set(`user_lang_${senderPsid}`, 'TAGALOG');
    } catch (e) {}
    await sendTextMessage(senderPsid, "🇵🇭 **Naka-set na sa Tagalog/Filipino!** Mula ngayon, sasagutin kita sa Tagalog/Taglish kahit mag-English ka man.", pageToken);
    return true;
  }
  ...
  if (['/auto', '/autolang'].includes(lowerText)) {
    ...
    await sendTextMessage(senderPsid, "🤖 **Smart Auto-Detect Enabled!** Kusa na ulit aangkop ang AI sa wikang gagamitin mo.", pageToken);
    return true;
  }

  // 🎨 Image Generator
  if (['/imagen', 'cmd_imagen', 'draw', 'image'].includes(lowerText)) {
    await sendTypingOn(senderPsid, pageToken);
    try {
      await kv.set(`user_mode_${senderPsid}`, 'IMAGE_MODE', { ex: 600 });
    } catch (e) {}
    await sendTextMessage(senderPsid, "🎨 **Image Generator Mode!**\n\nI-type mo na ngayon ang prompt o description ng picture na gusto mong ipagawa (halimbawa: *a cat wearing a snowcoat*).", pageToken);
...
async function handleEnglishTalkMode(senderPsid, userMessage, apiKeys, pageToken) {
  ...
  const tutorReply = await getDirectGeminiResponse(prompt, getRandomApiKey(apiKeys));
...
async function analyzeHomeworkWithGemini(imageUrl, apiKey, senderPsid) {
  try {
    ...
    let systemInstructionText = await getSystemInstructionForUser(senderPsid);
...
// Dynamic System Instruction para sa Language Preference
async function getSystemInstructionForUser(senderPsid) {
  let userLang = null;
  try {
    userLang = await kv.get(`user_lang_${senderPsid}`);
  } catch (e) {}

  let baseInstruction = "You are JepongDevxyz AI created by Jay-Ar Lee Espiritu. Helpful student tutor. ";

  if (userLang === 'ENGLISH') {
    return baseInstruction + "STRICT RULE: Respond strictly in English language ONLY. Even if the user talks in Tagalog or any other language, respond only in clean English.";
  } else if (userLang === 'TAGALOG') {
    return baseInstruction + "STRICT RULE: Respond strictly in Tagalog / Taglish language ONLY. Even if the user talks in English or any other language, respond in friendly conversational Tagalog/Taglish.";
  } else {
    return baseInstruction + "SMART AUTO DETECT: Always detect the language used by the user in their message (English, Tagalog, Taglish, Japanese, Spanish, etc.) and respond in that exact same language naturally.";
  }
}
